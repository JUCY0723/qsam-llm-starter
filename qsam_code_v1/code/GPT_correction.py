...
# 读取文件
df_train = robust_read_csv('sentiment_train_with_chatgpt_pred.csv')
df_pred = robust_read_csv('sentiment_test_with_chatgpt_pred.csv')
...
def build_features(df, top_words):
    ...
    df['chatgpt_pred_score_rolling'] = df['chatgpt_pred_score'].rolling(window=2, min_periods=1, center=True).mean()
    df['pos_cnt_x_len'] = df['pos_cnt'] * df['len']
    df['neg_cnt_x_word_count'] = df['neg_cnt'] * df['word_count']
    df['chatgpt_x_positive_ratio'] = df['chatgpt_pred_score'] * df['positive_ratio']
    df['chatgpt_sq'] = df['chatgpt_pred_score'] ** 2
    ...
    df['chatgpt_pred_score_std'] = df['chatgpt_pred_score'].rolling(window=3, min_periods=1, center=True).std().fillna(0)
...
feature_cols = [
    'chatgpt_pred_score', 'len', 'word_count', 'pos_cnt', 'neg_cnt', 'punct_cnt',
    'positive_ratio', 'negative_ratio', 'negation_cnt', 'topic_cnt', 'emotion_phrase_cnt',
    'chatgpt_pred_score_rolling', 'pos_cnt_x_len', 'neg_cnt_x_word_count', 'chatgpt_x_positive_ratio', 'chatgpt_sq',
    'unique_word_ratio', 'numeric_ratio', 'chatgpt_pred_score_std'
...
joblib.dump(voting, 'voting_super_calibrator_chatgpt.model')
print("Voting模型已保存为 voting_super_calibrator_chatgpt.model")
with open('top_words.pkl', 'wb') as f:
    pickle.dump(top_words, f)
print("top_words已保存为 top_words.pkl")

df_test['voting_super_calibrated'] = y_pred_test
df_test.to_csv('sentiment_test_super_calibrated_chatgpt.csv', index=False, encoding='utf-8-sig')
print('预测结果已保存为 sentiment_test_super_calibrated_chatgpt.csv')
