...
# 1. 参数配置
MODEL_PATH = 'models/bert_localpath'
BATCH_SIZE = 16
EPOCHS = 3
...
model = BertForSequenceClassification.from_pretrained(MODEL_PATH, num_labels=1, problem_type="regression")
...
print('BERT回归 MSE:', mean_squared_error(all_labels, all_preds))
print('BERT回归 MAE:', mean_absolute_error(all_labels, all_preds))
print('BERT回归 R2:', r2_score(all_labels, all_preds))
print('BERT回归 Pearson相关系数:', np.corrcoef(all_labels, all_preds)[0,1])

# 9. 保存结果
test_df['bert_pred_score'] = all_preds
test_df.to_csv('sentiment_test_with_bert_pred.csv', index=False, encoding='utf-8-sig')
print('BERT回归结果已保存至 sentiment_test_with_bert_pred.csv')
